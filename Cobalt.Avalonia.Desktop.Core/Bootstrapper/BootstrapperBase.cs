...
public abstract class BootstrapperBase : IDisposable
{
    ...
    /// <summary>
    /// Gets the service provider from the application host, providing access to registered services
    /// through the dependency injection container.
    /// </summary>
    /// <value>
    /// The <see cref="IServiceProvider"/> instance from the host's services collection.
    /// </value>
    /// <remarks>
    /// This property provides convenient access to the dependency injection container managed by
    /// the .NET Generic Host. It can be used to resolve services that have been registered during
    /// the <see cref="ConfigureServices"/> phase of the bootstrapper initialization.
    /// </remarks>
    /// <exception cref="InvalidOperationException">
    /// Thrown when the bootstrapper has not been started or the host is not available.
    /// Ensure <see cref="StartAsync"/> has been called and completed successfully before accessing this property.
    /// </exception>
    public IServiceProvider ServiceProvider =>
        Host?.Services ?? throw new InvalidOperationException("Bootstrapper is not started");
    ...
    {
        if (Disposed)
            throw new ObjectDisposedException(nameof(BootstrapperBase));
    }
